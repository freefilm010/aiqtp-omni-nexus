...
AS $$
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS auto_invest_allocation_repair_groups ON COMMIT DROP AS
  SELECT
  ...
  GET DIAGNOSTICS groups_repaired = ROW_COUNT;

  CREATE INDEX IF NOT EXISTS auto_invest_allocation_repair_groups_keeper_idx
    ON auto_invest_allocation_repair_groups (keeper_id);
  CREATE INDEX IF NOT EXISTS auto_invest_allocation_repair_groups_engine_symbol_idx
