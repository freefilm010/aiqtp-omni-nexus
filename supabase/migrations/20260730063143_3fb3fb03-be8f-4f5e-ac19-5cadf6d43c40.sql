...
-- 1. Repair: roll up duplicate active rows per (engine_id, asset_symbol).
WITH ranked AS (
  SELECT
    id,
    engine_id,
...
-- 2. Consolidation trigger: future INSERTs of active duplicates merge into the existing row.
...
-- 3. Unique partial index enforcing one active allocation per (engine_id, asset_symbol).
