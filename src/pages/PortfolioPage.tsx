...
const PortfolioPage = () => {
  ...
  const { realAssets, testAssets, netWorth } = useMemo(() => {
    if (!holdings) return { realAssets: [], testAssets: [], netWorth: { portfolio: 0, realAssetCount: 0, testAssetCount: 0 } };

    const active = holdings.filter((h) => h.quantity > 0);
    const valuations = active.map((h) => getValuation(h.symbol, h.quantity));
    const real = valuations.filter((v) => !v.isTestnet);
    const test = valuations.filter((v) => v.isTestnet);
    const portfolioVal = real.reduce((s, v) => s + v.valueUsd, 0);

    return {
      realAssets: real,
      testAssets: test,
      netWorth: { portfolio: portfolioVal, realAssetCount: real.length, testAssetCount: test.length },
    };
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <Tabs defaultValue="portfolio" className="space-y-6">
          ...
          <TabsContent value="portfolio">
            <Suspense fallback={<TabLoader />}><PortfolioAnalyticsDashboard /></Suspense>
          </TabsContent>
          <TabsContent value="heatmap">
            <Suspense fallback={<TabLoader />}><MarketHeatmap /></Suspense>
          </TabsContent>
          <TabsContent value="fundamentals">
            <Suspense fallback={<TabLoader />}><FundamentalAnalysis /></Suspense>
          </TabsContent>
        </Tabs>
      </main>
      <Footer />
